...
public class JobController {
	...
	public ResponseEntity<List<JobResponse>> getAllJobs() {
		...
		List<JobResponse> jobs = jobsMap.entrySet().stream().map(entry -> new JobResponse(entry.getKey().getName(),
				entry.getKey().getDuration().toSeconds(), entry.getValue())).collect(Collectors.toList());
